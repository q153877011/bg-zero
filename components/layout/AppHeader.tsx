  ...
  return (
    <header className={`${styles['app-header']} ${isScrolled ? styles['is-scrolled'] : ''}`}>
      <div className={styles['header-inner']}>
        ...
        {/* Right area — secondary nav + utilities */}
        <div className={styles['header-tail']}>
          <Link
            href="/blog"
            className={`${styles['tail-link']} ${blogActive ? styles['tail-link--active'] : ''}`}
            aria-current={blogActive ? 'page' : undefined}
          >
            <BookOpen size={13} className={styles['tail-link-ico']} />
            <span>{t('blog')}</span>
          </Link>
